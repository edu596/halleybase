...
$this->SetFillColor(9,139,162,255); //
    ...
    $this->RoundedRect($r1, $y1, ($r2 - $r1), $y2, 3.5, 'D', true);
    //$this->Line( $r1, $mid, $r2, $mid);
    $this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+1 );

    $this->SetFont( "Arial", "B", 11);
    $this->Cell(10,5, utf8_decode("R.U.C. ").$ruc, 0, 0, "C", false);

    $this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+6 );
    $this->SetFont( "Arial", "B", 11);
    $this->Cell(10,5, utf8_decode("BOLETA ELECTRÓNICA"), 0, 0, "C", false);

    $this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+11 );
    $this->SetFont( "Arial", "B", 11);
    $this->Cell(10,5,$num, 0,0, "C", false);
}
...
// Client address
function addClientAdresse($fecha,$cliente,$domicilio,$num_documento,$estado,$usuario, $guia, $tipopago, $nroreferencia, $moneda)
{
    $r1     = $this->w - 200;
    $r2     = $r1 + 68;
    $y1     = 28;
 ...
 // Client address
function addClientAdresse2($fecha,$cliente,$domicilio,$num_documento,$estado,$usuario, $guia, $tipopago, $nroreferencia)
{
    $r1     = $this->w - 190;
    $r2     = $r1 + 68;
    $y1     = 170; // Edicion
...
function addCols( $tab )
{
    ...
    $r2  = $this->w - ($r1 * 2) ;
    $y1  = 54;
    $y2  = $this->h - 55 - $y1; // + reduce el largo del cuadro
    $this->SetXY( $r1, $y1 );
...
function addCadreEurosFrancs2()
{
    ...
    $this->SetFont( "Arial", "B", 8);
    $this->SetXY( $r1, $y1+5 );
    $this->Cell(35,4, "T. DESCU. S/", 0, 0, "C");

    $this->SetFont( "Arial", "B", 8);
    $this->SetXY( $r1, $y1+12 );
    $this->Cell(35,4, "TOTAL S/", 0, 0, "C");
}
