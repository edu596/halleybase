...
function addSociete( $nom, $adresse,$logo,$ext_logo )
{
    ...
    $this->Image($logo , 18 ,8, 16 , 16 , $ext_logo);
    $this->SetXY( $x1, $y1 );
...
// Client address
function addClientAdresse($fecha,$cliente,$num_documento,$domicilio,$estado,$usuario,$guia, $tipopago)
{
    $r1     = $this->w - 190;
    $r2     = $r1 + 68;
    $y1     = 24;
 ...
 // Client address
function addClientAdresse2($fecha,$cliente,$num_documento,$domicilio,$estado,$usuario,$guia, $tipopago, $nroreferencia)
{
    $r1     = $this->w - 190;
    $r2     = $r1 + 68;
    $y1     = 170;
...
function addClientAdresseNC($cliente,$domicilio,$fechanc,$numfac,$fecfactemi, $observacion, $vendedorsitio, $ruccliente)
{
    $r1     = $this->w - 200;
    $r2     = $r1 + 68;
    $y1     = 24;
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $igv, $total, $moneda, $tdescuento, $ipagado, $saldo )
{
    ...
    $this->Cell( 1,4, number_format($total - $igv,2), '', '', 'R');
    ...
    $this->Cell( 1,4, number_format($total,2), '', '', 'R');

    $this->SetXY( $re, $y1+21 );
    //$this->Cell( 17,4, $moneda.sprintf("%0.2F", $total), '', '', 'R');
    $this->Cell( 1,4, number_format($ipagado,2), '', '', 'R');

    $this->SetXY( $re, $y1+25 );
    //$this->Cell( 17,4, $moneda.sprintf("%0.2F", $total), '', '', 'R');
    $this->Cell( 1,4, number_format($saldo,2), '', '', 'R');
...
function addTVAs2( $igv, $total, $moneda, $tdescuento, $ipagado, $saldo ) 
{
    $this->SetFont('Arial','',8);
