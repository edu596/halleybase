...
//$pdf->addSocietedireccion(htmlspecialchars_decode(utf8_decode("Dirección: ").$datose->domicilio_fiscal)); //Nuevo
...
$pdf->addClientAdresse(
    ...
    utf8_decode(htmlspecialchars_decode($regv->cliente)), 
    $regv->numero_documento, 
    utf8_decode(htmlspecialchars_decode($regv->direccion)), 
    $regv->estado, 
...
$pdf->addTVAs( $regv->sumatoria_igv_22_1 , $regv->subtotal,"S/ ", $regv->tdescuento, $regv->ipagado  , 
    $regv->saldo, $regv->icbper, $regv->otroscargos);
...
$pdf->addSociete2(utf8_decode("Teléfono: ").$datose->telefono1." - ".$datose->telefono2,
    "Email: ".$datose->correo,
...
////Enviamos los datos del cliente al método addClientAdresse de la clase Factura
$pdf->addClientAdresse2( $regv->fecha."   /  Hora: ".$regv->hora,    utf8_decode(htmlspecialchars_decode($regv->cliente)), $regv->numero_documento, utf8_decode($regv->direccion), $regv->estado, utf8_decode($regv->vendedorsitio), utf8_decode($regv->guia), utf8_decode($regv->tipopago), $letraCuota, $regv->moneda, $montocuota, $feccuota);
...
//Actualizamos el valor de la coordenada "y", que será la ubicación desde donde empezaremos a mostrar los datos
$y2= 206; // para el tamaño del cuadro del segundo detalle
...
$pdf->observSunat2($regv->numeracion_08,$regv->estado,$regv->hashc, $datose->webconsul, $datose->nresolucion);
 

//Mostramos el impuesto
$pdf->addTVAs2( $regv->sumatoria_igv_22_1, $regv->subtotal,"S/ ", $regv->tdescuento, $regv->ipagado  , 
    $regv->saldo , $regv->icbper, $regv->otroscargos );
$pdf->addCadreEurosFrancs2($regv->sumatoria_igv_22_1, $regv->nombretrib);

$pdf->AutoPrint();
//Linea para guardar la factura en la carpeta facturas PDF
$pdf->Output($regv->numeracion_08.'.pdf','I');
//$Factura=$pdf->Output('../facturasPDF/'.$regv->numeracion_08.'.pdf','F');
$pdf->Output($rutasalidafactura.$regv->numeracion_08.'.pdf','F');
