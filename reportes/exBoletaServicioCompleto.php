...
$rsptav = $servicioboleta->ventacabecera($_GET["id"], $_SESSION['idempresa']);
$datos = $servicioboleta->datosemp($_SESSION['idempresa']);
...
$rsptad = $servicioboleta->ventadetalle($_GET["id"]);
 $item=1;
while ($regd = $rsptad->fetch_object()) {
  $line = array( "ITEM"=> "$regd->norden",
                "DESCRIPCION"=> utf8_decode("$regd->descripcion"." - "."$regd->descdet"),
...
//set it to writable location, a place for temp generated PNG files
    ...
    $dataTxt=$regv->numero_ruc."|".$regv->tipo_documento_06."|".$regv->serie."|".$regv->numerofac."|".$regv->igv."|".$regv->Itotal."|".$regv->fecha2."|".$regv->tipo_documento."|".$regv->numero_documento."|";
...
$pdf->addTVAs2( $regv->igv , $regv->Itotal,"S/ ");
$pdf->addCadreEurosFrancs2($regv->igv);
